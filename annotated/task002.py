p=lambda g,k=67:-k*g or p([[r.pop()%sum(r[-1:],4)or k>>4&4for r in g]for _ in g],k-1)
# ----------------------------------------------------------------
# By having r be in the inner loop rather than the outer one, you can do a zipless rotation, since r.pop() mutates values.
p=lambda g,k=11:-k*g or[*map(lambda*b,d=0:[d:=c^c-0**k>>d&4for c in b][::-1],*p(g,k-1))]
# Honorable mention by oxjam
